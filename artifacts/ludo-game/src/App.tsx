...
import { ClerkStuckFallback } from './components/ClerkStuckFallback';
import { useClerkLoadTimeout } from './lib/useClerkLoadTimeout';
...
const clerkAppearance = {
  ...
  options: {
    ...
    logoImageUrl: `${window.location.origin}${basePath}/app-icon.png`,
  },
  variables: {
    colorPrimary: '#e0221c',
...
function SignInPage() {
  ...
  const stuck = useClerkLoadTimeout(isLoaded);

  if (stuck) return <ClerkStuckFallback context="Login পেজ লোড করা যাচ্ছে না।" />;
...
function SignUpPage() {
  ...
  if (stuck) return <ClerkStuckFallback context="Sign up পেজ লোড করা যাচ্ছে না।" />;
      ...
      <QueryClientProvider client={queryClient}>
        <TooltipProvider>
          <Switch>
            ...
            <Route path="/faq">{() => <PolicyPage kind="faq" />}</Route>
            <Route path="/help">{() => <PolicyPage kind="help" />}</Route>
          </Switch>
          <Toaster />
        </TooltipProvider>
      </QueryClientProvider>
