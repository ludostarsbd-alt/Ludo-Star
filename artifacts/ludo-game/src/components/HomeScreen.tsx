...
function SettingsScreen({
  ...
  const [sound, setSound] = useState(() => localStorage.getItem('ludo_pref_sound') !== 'off');
  const [music, setMusic] = useState(() => localStorage.getItem('ludo_pref_music') !== 'off');
  const [vibration, setVibration] = useState(() => localStorage.getItem('ludo_pref_vibration') === 'on');
  const { openUserProfile } = useClerk();

  useEffect(() => {
    localStorage.setItem('ludo_pref_sound', sound ? 'on' : 'off');
    localStorage.setItem('ludo_pref_music', music ? 'on' : 'off');
    localStorage.setItem('ludo_pref_vibration', vibration ? 'on' : 'off');
    window.dispatchEvent(new CustomEvent('ludo:preferences-changed', {
      detail: { sound, music, vibration },
    }));
  }, [sound, music, vibration]);

  const openPolicy = (path: string) => {
    const origin = window.location.origin.replace(/\/$/, '');
    window.open(`${origin}${basePath}${path}`, '_blank', 'noopener,noreferrer');
  };
    ...
    <ScreenShell activeNav="settings" onNavigate={k => onNavigate(k)}>
      ...
      <div className="px-4 w-full max-w-md mx-auto flex-1 flex flex-col gap-4 pb-6 overflow-y-auto">
        ...
        <div>
          ...
          <div className="flex flex-col gap-2">
            <button onClick={() => openPolicy('/privacy')} className={rowCls}>
            ...
            <button onClick={() => openPolicy('/help')} className={rowCls}>
            ...
            <button onClick={() => { window.location.href = 'mailto:ludostarsbd@gmail.com'; }} className={rowCls}>
            ...
            <button onClick={() => openPolicy('/faq')} className={rowCls}>
            ...
            <button onClick={() => openPolicy('/terms')} className={rowCls}>
