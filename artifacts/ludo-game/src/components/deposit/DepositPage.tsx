...
const BASE = API_ORIGIN; // backend API origin — for fetch() calls ONLY, never for app routing/Links
...
export function DepositPage({ onBack }: Props) {
  ...
  if (!isLoaded) {
    if (stuck) return <ClerkStuckFallback context="Store পেজ লোড করা যাচ্ছে না।" />;
    return (
