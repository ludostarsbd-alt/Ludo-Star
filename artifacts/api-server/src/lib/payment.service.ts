...
import { createSign, publicEncrypt, constants } from "node:crypto";
...
const NAGAD = {
  ...
  publicKey: process.env["NAGAD_PUBLIC_KEY"]?.replace(/\\n/g, "\n") ?? "",
  privateKey: process.env["NAGAD_PRIVATE_KEY"]?.replace(/\\n/g, "\n") ?? "",
  baseUrl: SANDBOX
...
export async function nagadInitiate(params: {
  ...
  if (!NAGAD.merchantId || !NAGAD.merchantKey || !NAGAD.publicKey || !NAGAD.privateKey) {
    throw new Error(
      "Nagad is not configured: merchant ID, merchant key, public key, and private key are required.",
    );
  }
...
async function nagadSign(data: string): Promise<string> {
  return createSign("RSA-SHA256").update(data, "utf8").sign(NAGAD.privateKey, "base64");
}
